...
public final class RyuDouble {
    ...
    public static int toString(double value, char[] result, int off) {
        long DOUBLE_MANTISSA_MASK = 4503599627370495L; // (1L << 52) - 1;
        int DOUBLE_EXPONENT_MASK = 2047; // (1 << 11) - 1;
        int DOUBLE_EXPONENT_BIAS = 1023; // (1 << (11 - 1)) - 1;
        long LOG10_5_NUMERATOR = 6989700L; // (long) (10000000L * Math.log10(5));
        long LOG10_2_NUMERATOR = 3010299L; // (long) (10000000L * Math.log10(2));
        ...
        if (e2 >= 0) {
            int q = Math.max(0, (int) (e2 * LOG10_2_NUMERATOR / 10000000L) - 1);
            // k = constant + floor(log_2(5^q))
            // q == 0 ? 1 : (int) ((q * 23219280L + 10000000L - 1) / 10000000L)
            int k = 122 + (q == 0 ? 1 : (int) ((q * 23219280L + 10000000L - 1) / 10000000L)) - 1;
            int i = -e2 + q + k;

            int actualShift = i - 3 * 31 - 21;
            validateShift(actualShift);

            int[] ints = POW5_INV_SPLIT[q];
            {
                dv = calculateLongValue(mv, actualShift, ints);
            }
            {
                dp = calculateLongValue(mp, actualShift, ints);
            }
            {
                dm = calculateLongValue(mm, actualShift, ints);
            }
            ...
            int q = Math.max(0, (int) (-e2 * LOG10_5_NUMERATOR / 10000000L) - 1);
            int i = -e2 - q;
            int k = (i == 0 ? 1 : (int) ((i * 23219280L + 10000000L - 1) / 10000000L)) - 121;
            int j = q - k;
        ...
        if (exp < 0) {
            ...
            index = insertZeroDot(result, index);
            for (int i = -1;i > exp;i--) {
                result[index++] = '0';
            }
            int current = index;
	...
	private static int insertZeroDot(char[] result, int index) {
		result[index++] = '0';
		result[index++] = '.';
		return index;
	}

    private static long calculateLongValue(long mv, int actualShift, int[] ints) {
        long mHigh = mv >>> 31;
    ...
    private static long insertDigit(char[] result, long output, int olength, int current, int i) {
        result[current + olength - i - 1] = (char) ('0' + output % 10);
        output /= 10;
        return output;
    }

    private static void validateShift(int actualShift) {
        if (actualShift < 0) {
